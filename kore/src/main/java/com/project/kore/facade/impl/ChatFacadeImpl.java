...
public class ChatFacadeImpl implements ChatFacade {
        ...
        return chats.stream()
                ...
                .filter(res -> {
                    if (res.getLastMessageTime() == null) {
                        ...
                        if (role == Role.CLIENT || role == Role.PERSONAL_TRAINER
                                || role == Role.NUTRITIONIST || role == Role.PSYCHOLOGIST) {
    ...
    private void validateChatPermission(User uA, User uB) {
        ...
        if (client != null && prof != null) {
            ...
            if (prof.getRole() == Role.PSYCHOLOGIST && client.getAssignedPsychologist() != null
                    && client.getAssignedPsychologist().getId().equals(prof.getId())) assigned = true;
        }
